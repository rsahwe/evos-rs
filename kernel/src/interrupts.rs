...
use x86_64::{
    PrivilegeLevel,
    instructions::{interrupts::enable, port::Port},
    registers::control::Cr2,
    set_general_handler,
    structures::{
        idt::{EntryOptions, ExceptionVector, InterruptDescriptorTable, InterruptStackFrame},
...
const PIC_SECOND_RANGE: RangeInclusive<PicInterrupt> =
    PicInterrupt::Cmos..=PicInterrupt::SecondaryAta;
...
impl TryFrom<u8> for PicInterrupt {
    ...
    fn try_from(interrupt: u8) -> Result<Self, Self::Error> {
        if interrupt.wrapping_sub(Pic::OFFSET) < 16 {
            // SAFETY: SAFE
            Ok(unsafe { transmute::<u8, PicInterrupt>(interrupt.wrapping_sub(Pic::OFFSET)) })
        } else {
            Err(())
        }
    }
        ...
        match ExceptionVector::try_from(index) {
            Ok(vector) => {
                match vector {
                    ExceptionVector::Page => {
                        if let Ok(fault_addr) = Cr2::read()
                            && frame
                                .stack_pointer
                                .as_u64()
                                .wrapping_sub(fault_addr.as_u64())
                                <= Size4KiB::SIZE
                        {
                            panic!(
                                "Kernel STACKOVERFLOW at rip 0x{:016x} detected!",
                                frame.instruction_pointer
                            );
                        }
                        panic!(
                            "kernel page fault e {} with frame:\n{:#?}\nand addr: {:?}",
                            error_code.unwrap(),
                            frame,
                            Cr2::read()
                        )
                    ...
                    _ => unreachable!(
                        "Unexpected interrupt with error {:?} {:?} with frame:\n{:#?}",
                        error_code, vector, frame
