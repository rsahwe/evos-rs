...
fn make_static_disk_from_folder<'a>(folder: impl Into<&'a str>) -> Box<[u8]> {
    ...
    let folder = PathBuf::from(folder_name).read_dir().unwrap_or_else(|_| {
        panic!(
            "Passed invalid folder {} to make_static_disk_from_folder",
            folder_name
        )
    });
    ...
    let files = folder
        .map(|file| {
            ...
            name.clone()
                .into_string()
                .unwrap_or_else(|_| panic!("Invalid file name {:?}", name))
        })
    ...
    let all = files
        .map(|file| {
            ...
            OpenOptions::new()
                .read(true)
                .open(PathBuf::from(folder_name).join(file.as_str()))
    ...
    let total_len = all
        .iter()
        .fold(0, |old, (name, content)| old + name.len() + content.len())
        + size_of::<usize>()
        + size_of::<usize>() * file_count * 3;

    let mut end_file = vec![0u8; total_len];

    let mut name_offset = end_file
        .as_mut_slice()
        .write(&file_count.to_le_bytes())
        .unwrap();
    let mut offset = name_offset + size_of::<usize>() * file_count * 3;
    for (name, file) in all {
        name_offset += (&mut end_file.as_mut_slice()[name_offset..])
            .write(&offset.to_le_bytes())
            .unwrap();
        name_offset += (&mut end_file.as_mut_slice()[name_offset..])
            .write(&name.len().to_le_bytes())
            .unwrap();
        name_offset += (&mut end_file.as_mut_slice()[name_offset..])
            .write(&file.len().to_le_bytes())
            .unwrap();
        offset += (&mut end_file.as_mut_slice()[offset..])
            .write(name.as_bytes())
            .unwrap();
        offset += (&mut end_file.as_mut_slice()[offset..])
            .write(file.as_slice())
...
fn main() {
    ...
    OpenOptions::new()
        ...
        .unwrap()
        .write_all(&file)
        .expect("Could not write ramdisk");

    let uefi_path = out_dir.join("uefi.img");
    bootloader::UefiBoot::new(&kernel)
        .set_ramdisk(&ramdisk_name)
